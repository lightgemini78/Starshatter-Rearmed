...
FighterAI::ExecFrame(double s)
{
	...
	if (ship->HullStrength() < 65 && rtb_code == 0 && !form_up || ship->GetFuelLevel() < 15 && rtb_code == 0 ) {
		...
		RadioMessage* msg = new(__FILE__,__LINE__) RadioMessage(ship, lead, RadioMessage::RTB);
...
FighterAI::ThrottleControl()
{
	...
	else if (rtb_code || farcaster) {			//****** Try to RTB FASTER
		desired = 1500; //750;							
...
//	tgt = ward;
	...
	if (tgt) {
		...
		if (objective.z < 0) {
			...
			if (gap < 3000) {        //8000						//******** DOnt go that far away
				Steer s;
