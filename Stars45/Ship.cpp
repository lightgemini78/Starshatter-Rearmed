...
friendly_fire_time(0), ward(0), net_observer_mode(false), orig_elem_index(-1)
{
	...
	es_jump     = design->escape_jump; //jump away
...
Ship::CheckShotIntersection(Shot* shot, Point& ipt, Point& hpt, Weapon** wep)
{
	...
	bool     hit_hull = false;
	bool     easy     = false;
	double	 breach	  = 5;			//** get piercing level

	if (shield)
	breach	 = shield->Piercing(shot);
	...
	if (shieldRep && ShieldStrength() > breach) {      //*** check if shot should pierce shields entirely.
...
Ship::Escapehull () 
{
	float es_hull = design->escape_hull;
	float deh= 0.25;
	if (es_hull >= 1 || es_hull <= 0 )
	    return deh;
