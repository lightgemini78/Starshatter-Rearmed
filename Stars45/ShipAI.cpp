...
ShipAI::ExecFrame(double secs)
{
	...
	if (target && target != ship->GetTarget()) {
		...
		if (target == ship->GetTarget() && target->Type() == SimObject::SIM_SHIP) { // && element_index == 1) {
...
ShipAI::FindObjective()
{
	...
	if (threat && !directed) {
		...
		if (WithDraw()) {
			Ship* control = ship->GetCarrier();
			if(control && element_index == 1 && last_sup_time > 15000) {			//** call carrier for help. It will send deployed fighters.
			RadioMessage* msg = new(__FILE__,__LINE__) RadioMessage(control, ship, RadioMessage::REQUEST_SUPPORT);
			RadioTraffic::Transmit(msg);
			last_sup_time = Game::GameTime();
			}
...
bool
ShipAI::WithDraw()				//****** Flee from superior numbers towards support ********
{
	bool withdraw = false;	
	double foe  = 0;							
	double frnd = 0;
	...
	while (++iter) {
		...
		if (contact && contact->Range(ship) < 80e3) {
			...
			if (unit && ship->IsHostileTo(unit) && unit->Class() == Ship::FIGHTER)       //***** Only fighters  count.(bombers will not scare fighters away)
				foe += unit->AIValue();
			else if (unit && unit->GetIFF() == ship->GetIFF() && unit->Class() < Ship::DESTROYER)  //***** Frigates and lesser count as inmediate support giver/in need.
				frnd += unit->AIValue();
		}
	}
