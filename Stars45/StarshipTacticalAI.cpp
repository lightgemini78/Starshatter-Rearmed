...
StarshipTacticalAI::FindThreat()
{
	...
	while (++iter) {
		...
		if (c_ship && c_ship != ship) {
			...
			double base_val = c_ship->AIValue();
...
{
	// see if carrier should bug out...   ****** Only use bool in shipdef for filtering.
	if (!ship || !c_ship || !ship->Escapejump() ) //&& ship->Class() != Ship::CARRIER && ship->Class() != Ship::SWACS ) 
	return;
	...
	double allowable_damage = ship->Design()->integrity * ship->Escapehull();  // damage taken to jump now not fixed, using value in ship def.
